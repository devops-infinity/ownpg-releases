...
This repository holds no source code. It is the public install, release, and issue-reporting hub for OwnPG, whose source lives in the private `devops-infinity/ownpg` repository. Its only tracked content is `README.md`, `SECURITY.md`, two license files, and three GitHub issue-form templates under `.github/ISSUE_TEMPLATE/`.
...
A GitHub Release on this repository, its binaries, `sha256.sum`, `sha256.sum.minisig`, and its SBOM, is written by the private `ownpg` repository's own release script, run by hand from that repository, never from here. This repository has no `.github/workflows/` directory and runs no CI of its own. The Homebrew formula for OwnPG does not live here either; it goes to the separate `devops-infinity/homebrew-tap` repository.
...
- Don't hand-create or hand-edit a GitHub Release, a release binary, a checksum file, or a signature here. The private `ownpg` repository's release process writes them, and a hand edit gets overwritten or falls out of sync with what that process expects.
